...
::: instructor
Optional challenge number 3 (see below; for visualizing the model) requires the command-line tool `dot` from Graphviz.
If `dot` is missing, you can either:

- show the solution, or
- try to install it.

The [installation instructions for Graphviz can be found here](https://graphviz.org/download/).
:::
...
Optionally, you can also visualize the same information as `model.summary()` a graph.
This step requires the command-line tool `dot` from Graphviz installed.
You may check that by executing `dot -V` in the command line. You should get something
as follows.
...
float32
```
The model weights are represented using `float32` datatype, which consumes 4 byte each when loaded into the computer's memory.
...
{alt='A directed graph showing the three layers of the neural network connected by arrows. First layer is an InputLayer of shape. Second layer is of type Dense with a relu activation. The third layer is also of type Dense, with a softmax activation. The input and output shapes of every layer is also mentioned. Only the second and third layers contain trainable parameters.'}
